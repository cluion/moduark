...
final class ModuleMakeCommandTest extends TestCase
{
    ...
    public function test_it_rejects_an_unsupported_maker_type(): void
    {
        $this->command('moduark:make User view ProfileView')
            ->expectsOutputToContain(
                'Module Maker failed: Maker type [view] is not supported; expected cast, channel, class, controller, enum, event, exception, factory, interface, job, job-middleware, listener, mail, middleware, migration, model, notification, observer, policy, request, resource, rule, scope, seeder, or trait.',
            )
            ->assertExitCode(ExitPolicy::TOOL_ERROR);
    }
