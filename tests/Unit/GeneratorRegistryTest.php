...
final class GeneratorRegistryTest extends TestCase
{
    public function test_it_resolves_case_insensitively_and_lists_canonical_ids_in_order(): void
    {
        ...
        self::assertSame(
            ['cast', 'class', 'controller', 'enum', 'exception', 'factory', 'interface', 'middleware', 'model', 'policy', 'request', 'resource', 'rule', 'scope', 'seeder', 'trait'],
            array_map(
    ...
    public function test_unknown_ids_preserve_the_public_maker_error(): void
    {
        ...
        $this->expectExceptionMessage(
            'Maker type [view] is not supported; expected cast, class, controller, enum, exception, factory, interface, middleware, model, policy, request, resource, rule, scope, seeder, or trait.',
        );
