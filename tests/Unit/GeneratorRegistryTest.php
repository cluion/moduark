...
final class GeneratorRegistryTest extends TestCase
{
    public function test_it_resolves_case_insensitively_and_lists_canonical_ids_in_order(): void
    {
        ...
        self::assertSame(
            ['cast', 'channel', 'class', 'component', 'controller', 'enum', 'event', 'exception', 'factory', 'interface', 'job', 'job-middleware', 'listener', 'mail', 'middleware', 'migration', 'model', 'notification', 'observer', 'policy', 'request', 'resource', 'rule', 'scope', 'seeder', 'test', 'trait', 'view'],
            array_map(
    ...
    public function test_unknown_ids_preserve_the_public_maker_error(): void
    {
        ...
        $this->expectExceptionMessage(
            'Maker type [verification] is not supported; expected cast, channel, class, component, controller, enum, event, exception, factory, interface, job, job-middleware, listener, mail, middleware, migration, model, notification, observer, policy, request, resource, rule, scope, seeder, test, trait, or view.',
        );
