...
LC1-H adds the Preview, default-read-only `moduark:export-set` plan. Repeatable
...
plans use exit `0`, blockers exit `1`, and invalid mappings exit `2`.

LC1-I adds the Preview `--materialize` opt-in. Every package is prepared before
any target is published, targets are rechecked, and dependency-ordered renames
publish the set. Ordinary failures roll back newly published targets; schema
