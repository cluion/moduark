...
final class InvalidArchitectureConfiguration extends InvalidArgumentException
{
    ...
    {
        return new self('The modules.architecture.rules configuration must use rule IDs as string keys.');
    }

    public static function unknownRule(string $rule): self
    {
        return new self("The modules.architecture.rules configuration contains unknown rule [{$rule}].");
    }

    public static function booleanOverride(string $rule, mixed $value): self
    {
        return new self(sprintf(
            'The modules.architecture.rules.%s configuration must be a boolean; received %s.',
            $rule,
